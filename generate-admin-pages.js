...
function generateAdminPage(config) {
  // Convert field names to database format (camelCase to snake_case)
  const toSnakeCase = (str) => str.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`);
  
  // Generate interface fields
  const interfaceFields = config.fields.map(field => 
    `  ${field}: ${field === 'nightVision' || field === 'poe' ? 'boolean' : field.includes('channels') || field.includes('battery') && field.includes('Life') ? 'string' : 'string'};`
  ).join('\n');

  // Generate default form data
  const defaultFormData = config.fields.map(field => 
    `    ${field}: '${config.defaultValues[field]}',`
  ).join('\n');

  // Generate field mappings for fetch
  const fieldMappings = config.fields.map(field => 
    `          ${field}: p.${toSnakeCase(field)},`
  ).join('\n');

  // Generate form body fields
  const formBodyFields = config.fields.map(field => {
    const dbField = toSnakeCase(field);
    if (field === 'channels') {
      return `          ${dbField}: parseInt(formData.${field}),`;
    } else if (field === 'nightVision' || field === 'poe') {
      return `          ${dbField}: formData.${field} === 'true',`;
    } else {
      return `          ${dbField}: formData.${field},`;
    }
  }).join('\n');

  // Generate table headers
  const tableHeaders = config.fields.map(field =>
    `              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">${config.fieldLabels[field]}</th>`
  ).join('\n');

  // Generate table cells
  const tableCells = config.fields.map(field => {
    if (field === 'nightVision' || field === 'poe') {
      return `                <td className="px-6 py-4 text-sm text-gray-900">{product.${field} ? 'Yes' : 'No'}</td>`;
    } else {
      return `                <td className="px-6 py-4 text-sm text-gray-900">{product.${field}}</td>`;
    }
  }).join('\n');

  // Generate form inputs
  const formInputs = config.fields.map(field => {
  ...
  // Generate edit form data mapping
  const editFormData = config.fields.map(field => {
    if (field === 'nightVision' || field === 'poe') {
      return `      ${field}: product.${field}.toString(),`;
...
export default function ${config.title.replace(/\s+/g, '')}Admin() {
...
${defaultFormData}
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/${config.api}?admin=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProduct
        ? \`/api/${config.api}?id=\${editingProduct.id}\`
        : '/api/${config.api}';
...
${editFormData}
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(\`/api/${config.api}?id=\${id}\`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">${config.title} Products</h1>
          <p className="text-gray-600 mt-1">Manage your ${config.title} products</p>
        </div>
        <button
...
categories.forEach(config => {
  const dirPath = path.join(__dirname, 'app', 'admin', 'categories', config.name);
  ...
  const content = generateAdminPage(config);
