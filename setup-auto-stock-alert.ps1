...
try {
    ...
    $host = ($dbInfo[0] -split ':')[0]
    $port = ($dbInfo[0] -split ':')[1]
    $database = $dbInfo[1]
    $username = (($DATABASE_URL -split '://')[1] -split ':')[0]

    Write-Host "Connecting to database: $database@$host" -ForegroundColor Gray
    
    Get-Content $sqlFile | psql -h $host -p $port -U $username -d $database -f - 2>&1 | ForEach-Object {
...
try {
    $result = $checkScript | psql -h $host -p $port -U $username -d $database -t -A 2>&1
    ...
    Write-Host "⚠ Could not verify table creation" -ForegroundColor Yellow
}
