...
export async function POST(request: Request) {
  try {
    const { dealerId, message } = await request.json();
    ...
    await sendStockAlertEmail({
      ...
      sentByRole: 'admin',
      sentByLabel: 'Administrator',
      customMessage: message || null,
    });

    await pool.query(
      `INSERT INTO dealer_notifications (dealer_id, title, message, type, priority, sent_via_email, email_sent_at, created_by)
       VALUES ($1, $2, $3, 'stock_alert_pdf', 'high', TRUE, NOW(), 'admin')`,
      [
        dealer.dealer_id,
        'Stock Alert Report Sent',
        'Admin sent a PDF stock alert report covering low-stock and out-of-stock products to your email.',
      ]
    );
