...
export default function FourGSimCameraPage() {
  ...
  // Dynamic storage options from database
  const storageOptions = useMemo(() => {
    return quotationSettings?.storage?.map((s: any) => s.capacity) || ["32GB", "64GB", "128GB", "256GB"];
  }, [quotationSettings]);
  ...
  const [selectedPowerSupply, setSelectedPowerSupply] = useState<string[]>([]);
  const [selectedSimSupport, setSelectedSimSupport] = useState<string[]>([]);
    ...
    try {
      ...
      if (data.success) {
        ...
        const mappedProducts = data.products.map((p: any) => ({
          ...
          simSupport: p.sim_support,
          storage: p.storage,
          powerSupply: p.power_supply,
      ...
      <main className="flex-1 pt-24 pb-16">
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600 mb-6">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-slate-900 font-medium">4G SIM Camera</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="lg:w-64 flex-shrink-0">
              <div className="bg-white rounded-lg shadow-sm p-4 lg:p-6 lg:sticky lg:top-24">
                ...
                {/* Power Supply Filter */}
                <FilterSection title="Power Supply" sectionKey="powerSupply">
                  {powerSupplyOptions.map(power => (
                    <label key={power} className="flex items-center gap-2 cursor-pointer group">
                      <Checkbox
                        checked={selectedPowerSupply.includes(power)}
                        onCheckedChange={() => toggleFilter(power, selectedPowerSupply, setSelectedPowerSupply)}
                      />
                      <span className="text-sm text-slate-700 group-hover:text-slate-900">
                        {power}
                      </span>
                ...
                {/* SIM Support Filter */}
                <FilterSection title="SIM Support" sectionKey="simSupport">
                  {simSupportOptions.map(sim => (
                    <label key={sim} className="flex items-center gap-2 cursor-pointer group">
                      <Checkbox
                        checked={selectedSimSupport.includes(sim)}
                        onCheckedChange={() => toggleFilter(sim, selectedSimSupport, setSelectedSimSupport)}
                      />
                      <span className="text-sm text-slate-700 group-hover:text-slate-900">
                        {sim}
                      </span>
                  ...
                  {storageOptions.map(storage => (
                    <label key={storage} className="flex items-center gap-2 cursor-pointer group">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleFilter(storage, selectedStorage, setSelectedStorage)}
                      />
                      <span className="text-sm text-slate-700 group-hover:text-slate-900">
                        {storage}
                      </span>
                    ...
                    >
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="mb-3 space-y-1.5">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">SIM Support:</span>
                            <span className="text-slate-600">{product.simSupport}</span>
                          </div>
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">Storage:</span>
                            <span className="text-slate-600">{product.storage}</span>
                          </div>
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">Power:</span>
                            <span className="text-slate-600">{product.powerSupply}</span>
                          </div>
                        </div>
