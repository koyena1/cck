...
export default function BodyWornCameraPage() {
  ...
  const resolutions = useMemo(() => {
    return quotationSettings?.pixels?.map((p: any) => p.name) || ["1080P", "2K", "4K"];
  }, [quotationSettings]);
  ...
  const filteredProducts = useMemo(() => {
    const filtered = products.filter(product => {
      ...
      if (selectedFeatures.length > 0 && !selectedFeatures.includes(product.features)) return false;
      ...
      <main className="flex-1 pt-24 pb-16">
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600 mb-6">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-slate-900 font-medium">Body Worn Camera</span>
          </div>
                  ...
                  {featuresOptions.map(feature => (
                    <label key={feature} className="flex items-center gap-2 cursor-pointer group">
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleFilter(feature, selectedFeatures, setSelectedFeatures)}
                      />
                      <span className="text-sm text-slate-700 group-hover:text-slate-900">
                        {feature}
                      </span>
                    ...
                    >
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="mb-3 space-y-1.5">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">Features:</span>
                            <span className="text-slate-600">{product.features}</span>
                          </div>
                        </div>
