...
function IpComboContent() {
  ...
  const channels = useMemo(() => {
    return quotationSettings?.channels?.map((ch: any) => ch.channel_count) || [4, 8, 16];
  }, [quotationSettings]);
  
  // Dynamic HDD/storage options from database
  const hddOptions = useMemo(() => {
    return quotationSettings?.storage?.map((s: any) => s.capacity) || ["1TB", "2TB", "4TB", "6TB", "8TB"];
  }, [quotationSettings]);
  
  // Dynamic resolutions/pixels from database
  const resolutions = useMemo(() => {
    return quotationSettings?.pixels?.map((p: any) => p.name) || ["2MP", "3MP", "4MP", "5MP", "8MP"];
  }, [quotationSettings]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      const res = await fetch(`/api/ip-combo-products?t=${timestamp}`, {
        cache: 'no-store',
        headers: {
      ...
      console.log('📦 IP Combo API Response:', data);
  ...
  const filteredProducts = useMemo(() => {
    ...
    console.log('📦 IP Combo Filtering:');
      ...
      <main className="flex-1 pt-24 pb-16">
        <div className="container mx-auto px-4">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600 mb-6">
            ...
            <ChevronRight className="w-4 h-4" />
            <span className="text-slate-900 font-medium">IP Combo</span>
          </div>

          {/* Page Header */}
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-2">
              {selectedBrand ? `${selectedBrand} ` : ''}IP Combo Kits
            </h1>
                ...
                <div className="lg:block">
                  ...
                  {selectedBrand && (
                    <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
                      <p className="text-xs text-slate-600 mb-1">Selected Brand</p>
                      <p className="font-bold text-slate-900">{selectedBrand}</p>
                      <button
                        onClick={() => router.push('/categories')}
                        className="text-xs text-[#e63946] hover:underline mt-2"
                      >
                        Change Brand
                      </button>
                    </div>
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 lg:block lg:overflow-visible lg:pb-0 scrollbar-hide">
                    {/* Channel Filter - Only 4, 8, 16 */}
                    <FilterSection title="Channels" sectionKey="channel">
                      {channels.map(channel => (
                        <label key={channel} className="flex items-center gap-2 cursor-pointer group">
                          <Checkbox
                            checked={selectedChannels.includes(channel)}
                            onCheckedChange={() => toggleFilter(channel, selectedChannels, setSelectedChannels)}
                          />
                    ...
                    {/* HDD Filter */}
                    <FilterSection title="HDD Storage" sectionKey="hdd">
                      {hddOptions.map(hdd => (
                        <label key={hdd} className="flex items-center gap-2 cursor-pointer group">
                          <Checkbox
                            checked={selectedHDD.includes(hdd)}
                            onCheckedChange={() => toggleFilter(hdd, selectedHDD, setSelectedHDD)}
                          />
                          <span className="text-sm text-slate-700 group-hover:text-slate-900">
                            {hdd}
                          </span>
                    ...
                    >
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="mb-3 space-y-1.5">
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">Channels:</span>
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">HDD:</span>
                            <span className="text-slate-600">{product.hdd}</span>
                          </div>
                          <div className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-slate-700 min-w-[70px]">Cable:</span>
                            <span className="text-slate-600">{product.cable}</span>
                          </div>
                        </div>
...
export default function IpComboPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading IP Combo Products...</p>
        </div>
      </div>
    }>
      <IpComboContent />
