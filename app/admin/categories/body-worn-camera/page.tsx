...
export default function BodyWornCameraAdmin() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/body-worn-camera-products?admin=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProduct
        ? `/api/body-worn-camera-products?id=${editingProduct.id}`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/body-worn-camera-products?id=${id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Body Worn Camera Products</h1>
          <p className="text-gray-600 mt-1">Manage your Body Worn Camera products</p>
        </div>
        <button
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Battery Life</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {products.map((product) => (
              <tr key={product.id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-900">{product.batteryLife}</td>
                <td className="px-6 py-4 text-sm text-gray-900">{product.storage}</td>
