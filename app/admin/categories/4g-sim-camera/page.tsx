...
export default function FourGSIMCameraAdmin() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/sim-4g-camera-products?admin=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProduct
        ? `/api/sim-4g-camera-products?id=${editingProduct.id}`
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          sim_support: formData.simSupport,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/sim-4g-camera-products?id=${id}`, {
        method: 'DELETE'
      });
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex items-center gap-3">
          <CategoryExcelUpload category="4g-sim-camera" onUploaded={fetchProducts} />
          <button
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={formData.simSupport}
                    onChange={(e) => setFormData({ ...formData, simSupport: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    required
                  />
                </div>
