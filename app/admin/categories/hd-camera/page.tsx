...
export default function HDCameraAdmin() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/hd-camera-products?admin=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProduct
        ? `/api/hd-camera-products?id=${editingProduct.id}`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/hd-camera-products?id=${id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">HD Camera Products</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-1">Manage your HD Camera products</p>
        </div>
        <div className="flex items-center gap-3">
          <CategoryExcelUpload category="hd-camera" onUploaded={fetchProducts} />
          <button
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead className="bg-gray-50 dark:bg-slate-800">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Lens</th>
          ...
          <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
            {products.map((product) => (
              <tr key={product.id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{product.lens}</td>
