...
export default function SolarCameraAdmin() {
  ...
  const fetchProducts = async () => {
    try {
      const res = await fetch('/api/solar-camera-products?admin=true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingProduct
        ? `/api/solar-camera-products?id=${editingProduct.id}`
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          solar_panel: formData.solarPanel,
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/solar-camera-products?id=${id}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Solar Camera Products</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-1">Manage your Solar Camera products</p>
        </div>
        <div className="flex items-center gap-3">
          <CategoryExcelUpload category="solar-camera" onUploaded={fetchProducts} />
          <button
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          <thead className="bg-gray-50 dark:bg-slate-800">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Solar Panel</th>
          ...
          <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
            {products.map((product) => (
              <tr key={product.id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">{product.solarPanel}</td>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, solarPanel: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    required
                  />
                </div>
